var TableCustomerDatatablesAjax = function () {
	var table_container=$("#customer_container");
	var ajax_url="customer_get_ajax.php";
	var modification_url="#";
	var show_count=100;
	var query="";
	var page=1;
    var handleTable = function () {
		var table=$("#customer_table_id");
		var table_container=$("#customer_container");
		var pagination_buttons=$(table_container).find('.customer_pagination').children("li.paginate_button").find("a");
		var searchBtn=$(table_container).find('button.searchBtn');
		var clearBtn=$(table_container).find('button.clearBtn');
		var editBtn=$(table_container).find('a.customer_table_id_edit');
		var deleteBtn=$(table_container).find('a.customer_table_id_delete');
		var activeBtn=$(table_container).find('a.customer_table_id_active');
		var deactiveBtn=$(table_container).find('a.customer_table_id_deactive');
		...
		$(searchBtn).on('click',function(){
			query=$("#customer_search_input").val();
			getDataFromAJAX();
		});
