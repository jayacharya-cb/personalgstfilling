<?php
$page_id                     ="514";
$page_title                     ="Create Reverse Sales Invoice";
include("connect.php");
...
$customer_gstin              ="";
$customer_place_of_supply    ="GUJARAT";
...
if(isset($_POST['save_invoice']))
{
			...
			if($customer_name!="")
			{
					...
					if($QuickInvoiceInfoId!=0)
					{
							...
							foreach($Items as $Item)
							{
								...
								if($item_description!="")
								{
									...
									$item_unit_cost=$Item['purchase_price'];
									$item_subtotal=$Item['total_val'];
									$total_tax_rate=$Item['cgst_rate']+$Item['sgst_rate']+$Item['igst_rate']+$Item['cess_rate'];
									$item_taxable_value=$item_subtotal/(1+($total_tax_rate/100));
									$item_discount_val=$item_unit_cost-$item_subtotal;
									if($item_discount_val<0)
									$item_discount_val=0;
									$item_discount_rate=($item_discount_val*100)/$item_subtotal;		
									$Item['discount']=($item_taxable_value*$item_discount_rate)/100;
									$Item['unit_price']=$db->round(($item_taxable_value+$Item['discount'])/$Item['quantity']);
									
									$item_type                         =$Item['gst_type'];
									$item_gst_code                     =$Item['gst_code'];
									$item_code                 		   ="";
									$item_qty                          =floatval($Item['quantity']);
									$item_unit_price                   =floatval($Item['unit_price']);// Not Available Directly
									$item_unit_of_messurement          ="";
									$item_discount                     =$db->round(floatval($Item['discount']));// Not Available Directly
									$item_note              		   ="";
				...
				else
				{
					...
					if($mode!="edit")
					{
						...
						$db->rp_location("quick_invoice_reverse_crud.php?m=a");
					}
					else
					{
						$_SESSION['last_submitted']=$_POST;
						$db->rp_location("quick_invoice_reverse_crud.php?m=e&iid=".$QuickInvoiceInfoId);
					}
				}
			}
			else
	...
	if($reply['ack']==1)
	{
		...
		$db->rp_location("quick_invoice_reverse_view.php?m=e&iid=".$QuickInvoiceInfoId);
	}
	else
	{
...
if($_REQUEST['mode']=="edit")
{
	...
	catch(Exception $e)
	{
		...
		$db->rp_location("quick_invoice_reverse_manage.php");
	}
}
                                ...
                                <div class="portlet light portlet-fit bordered">
                                    <div class="portlet-title">
                                        <div class="caption">
                                            ...
                                            <span class="caption-subject font-red sbold uppercase">Reverse Invoice</span>
                                        </div>
										<div class="actions">
										
											<div class="btn-group btn-group-devided">
													<a name="options" href="quick_invoice_reverse_manage.php" class="btn btn-transparent blue btn-outline btn-square btn-sm active" id="back-invoice-btn" type="button">Back</a>
											...
											<tbody class="line_items">
												<?php 
															...
															<tr class="error-container<?php echo $row_count ?>" data-count="<?php echo $row_count ?>">
																...
																<td>
																	<input autocomplete="off" id="line_items[<?php echo $row_count ?>][unit_price]" name="line_items[<?php echo $row_count ?>][unit_price]" disabled="disabled"  placeholder="" class="form-control table-input text-right numeric-input item-rate" type="text" value="<?php echo $item['item_unit_price']; ?>">
																</td>
																<td>
																	<input autocomplete="off" id="line_items[<?php echo $row_count ?>][purchase_price]" name="line_items[<?php echo $row_count ?>][purchase_price]"  placeholder="" class="form-control table-input text-right numeric-input item-purchase-rate" type="text" value="<?php echo $item['item_unit_cost']; ?>" data-validation="required">
																</td>
																<td>
																...
																<td>
																	<input autocomplete="off" id="line_items[<?php echo $row_count ?>][discount]" name="line_items[<?php echo $row_count ?>][discount]" disabled  placeholder="" class="form-control table-input text-right numeric-input item-discount" type="text" value="<?php echo $item['item_discount']; ?>">
																</td>
																<td class=""> 
																...
																<td>
																	<input autocomplete="off" id="line_items[<?php echo $row_count ?>][total_val]" name="line_items[<?php echo $row_count ?>][total_val]"  placeholder="" class="form-control table-input text-right numeric-input item-subtotal" type="text" value="<?php echo $item['item_subtotal']; ?>" data-validation="required">
																</td>
															</tr>
...
$(function(){ 
  ...
  $.validate({
		form : '#invoice-form', 
		 ...
		 inlineErrorMessageCallback: function($el, errorMessage, config) {
 ...
 function assignAutoComplete(node) {
		 $(node).autocomplete({	
			  ...
			  select: function( event, ui ) {
				...
				var unit_cost_input=$(parent).find('input[name="line_items['+index+'][purchase_price]"]');
				$(unit_cost_input).val(ui.item.unit_cost);
 ...
 function createNewRow(row_count){
	 var tableRow='<tr class="error-container'+row_count+'" data-count="'+row_count+'">'+
					...
					'<td><input autocomplete="off" id="line_items['+row_count+'][unit_price]" name="line_items['+row_count+'][unit_price]" value="" placeholder="" disabled="disabled" class="form-control table-input text-right numeric-input item-rate" type="text"></td>'+
					'<td><input autocomplete="off" id="line_items['+row_count+'][purchase_price]" name="line_items['+row_count+'][purchase_price]" value="" placeholder="" class="form-control table-input text-right numeric-input item-purchase-rate" type="text"></td>'+
					...
					'<td><input autocomplete="off" id="line_items['+row_count+'][discount]" name="line_items['+row_count+'][discount]" value="" placeholder="" disabled="disabled" class="form-control table-input text-right numeric-input item-discount" type="text"></td>'+
					...
					'</td>'+
					'<td>'+
					'<input autocomplete="off" id="line_items['+row_count+'][total_val]" name="line_items['+row_count+'][total_val]" value="" placeholder="" class="form-control table-input text-right numeric-input item-subtotal" type="text">'
					'</td>'+
				'</tr>';
 ...
 function calculateTotal(){
	 ...
	 $(".line_items").find("tr").each(function(){
		...
		total_cess=0;
		 ...
		 var unit_price_input=$(parent).find('input[name=line_items\\['+index+'\\]\\[unit_price\\]]');
		 var qty_input=$(parent).find('input[name=line_items\\['+index+'\\]\\[quantity\\]]');
		 var discount_input=$(parent).find('input[name=line_items\\['+index+'\\]\\[discount\\]]');
		 var stotal_input=$(parent).find('input[name=line_items\\['+index+'\\]\\[stotal\\]]');
		 ...
		 var item_trade_rate_input=$(parent).find('input[name=line_items\\['+index+'\\]\\[purchase_price\\]]');
		 var total_val=$(grand_total_input).val();
		 total_val=(total_val!="")?aj.roundNumber(total_val):0;
		 var total_tax_rate=0;
		 var item_trade_rate=$(item_trade_rate_input).val();
		 item_trade_rate=(item_trade_rate!="")?item_trade_rate:0;
		 
		 
		 var cgst_val_input=$(parent).find('input[name=line_items\\['+index+'\\]\\[cgst_val\\]]');
		 var sgst_val_input=$(parent).find('input[name=line_items\\['+index+'\\]\\[sgst_val\\]]');
		 var igst_val_input=$(parent).find('input[name=line_items\\['+index+'\\]\\[igst_val\\]]');
		 var cess_val_input=$(parent).find('input[name=line_items\\['+index+'\\]\\[cess_val\\]]');
		 var cgst_rate=0;			 
		 var sgst_rate=0;			 
		 var igst_rate=0;			 
		 var cess_rate=0;			 
		 ...
		 if(place_of_supply=="GUJARAT")
		 {
			 ...
			 total_tax_rate=parseFloat(cgst_rate)+parseFloat(sgst_rate);
		 }
		 else
		 {
			 ...
			 total_tax_rate=parseFloat(igst_rate);
			 
		 
		}
		 var cess_rate_input=$(parent).find('input[name=line_items\\['+index+'\\]\\[cess_rate\\]]');
		 var cess_rate=$(cess_rate_input).val();
		 var cess_rate=(cess_rate!="")?parseFloat(cess_rate):0;
		
		 
		 total_tax_rate=total_tax_rate+cess_rate;
		 var total_taxable=aj.roundNumber(total_val/(1+(total_tax_rate/100)));
		 $(total_taxable_input).val(total_taxable);
		 
		  var cgst_val=aj.roundNumber((total_taxable*cgst_rate)/100);
		  $(cgst_val_input).val(cgst_val);			 
		  var sgst_val=aj.roundNumber((total_taxable*sgst_rate)/100);
		  $(sgst_val_input).val(sgst_val);
		  var  igst_val=aj.roundNumber((total_taxable*igst_rate)/100);
		  $(igst_val_input).val(igst_val)	 	
		  var  cess_val=aj.roundNumber((total_taxable*cess_rate)/100);
		  $(cess_val_input).val(cess_val)
		 
		 // Origanl Discount
		 
		 var origanl_discount=item_trade_rate-grand_total;
		 origanl_discount=(origanl_discount>0)?origanl_discount:0;
		 // How Many Percentage of grand_total
		 var origanl_discount_percentage=0;
		 if(grand_total!=0)
		 {
			 origanl_discount_percentage=(origanl_discount*100)/grand_total
		 }
		 
		 var taxable_discount=aj.roundNumber((total_taxable*origanl_discount_percentage)/100)
		 $(discount_input).val(taxable_discount);
		 
		 var unit_price_for_all_qty=aj.roundNumber(total_taxable+taxable_discount);
		 var unit_price_for_one_qty=aj.roundNumber(unit_price_for_all_qty/qty);
		 $(unit_price_input).val(unit_price_for_one_qty);
		 $(stotal_input).val(unit_price_for_all_qty);
 ...
 function checkStock(){
	 ...
	 $(".line_items").find("tr").each(function(){
		var index=$(this).data("count");
		var parent=$(this);
		var description_input=$(parent).find('input[name="line_items['+index+'][description]"]');
		if($(description_input).val()!="")
		{
			
		var quantity_input=$(parent).find('input[name="line_items['+index+'][quantity]"]');
		var subtotal_input=$(parent).find('input[name="line_items['+index+'][s_total]"]');
		var pprice_input=$(parent).find('input[name="line_items['+index+'][purchase_price]"]');
		...
		if($(subtotal_input).val()=="")
		{
			var _popover;
			var message="Enter some value";
			_popover = $(subtotal_input).popover({
				trigger: "manual,click",
				placement: "top",
				content: message,
				template: "<div class=\"popover\"><div class=\"arrow\"></div><div class=\"popover-inner\"><div class=\"popover-content\"><p></p></div></div></div>"
			...
			//_popover.data("popover").options.content = message;
			//$(quantity_input).popover('show');
			$(subtotal_input).addClass('has-error');
			valid=false;
		}
		else
		{
			$(subtotal_input).removeClass('has-error');
			$(subtotal_input).popover("hide");
			$(subtotal_input).popover("destroy");
		}
		...
		if($(pprice_input).val()=="")
		{
			...
			//_popover.data("popover").options.content = message;
			//$(quantity_input).popover('show');
			$(pprice_input).addClass('has-error');
			valid=false;
		}
		else
		{
			$(pprice_input).removeClass('has-error');
			$(pprice_input).popover("hide");
			$(pprice_input).popover("destroy");
		}
		}
	});
	return valid;
