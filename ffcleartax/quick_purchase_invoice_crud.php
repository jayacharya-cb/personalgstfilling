...
include("connect.php");
$ctableQuickPurchaseInvoiceInfo="quick_purchase_invoice_info";
$ctableQuickPurchaseInvoiceItem="quick_purchase_invoice_item";
...
$next_number=intval($db->getlastInsertId($ctableQuickPurchaseInvoiceInfo)+1);
$invoice_serial_no           =$db->generateSerialWithPrefix(PURCHASE_INVOICE_SERIAL_PREFIX,$next_number,4);
...
if(isset($_POST['save_invoice']))
{
	...
	if(!empty($Items))
	{
		try
		{
			if($customer_name!="")
			{
				if($customer_id!="")
				{
					$customerInfo=$db->rp_getData("quick_invoice_contact","*","id='".$customer_id."'","",0);
					if($customerInfo)
					{
						$customerInfo=mysql_fetch_assoc($customerInfo);
						$customer_billing_address=$customerInfo['address'];
						$customer_billing_pincode=$customerInfo['zipcode'];
						$customer_billing_city=$customerInfo['city'];
						$customer_billing_state=$customerInfo['state'];
						$customer_billing_country=$customerInfo['country'];
					}
				...
				if($mode!="edit")
				{
					...
					if($invoice_serial_no!="" )
					{
						...
						$QuickInvoiceInfoId                        =$db->rp_insert($ctableQuickPurchaseInvoiceInfo,$value_invoice,$columns_invoice);
					}
					else
					{
				...
				else
				{
					...
					=array("customer_id"=>$customer_id,
							...
							$db->rp_update($ctableQuickPurchaseInvoiceInfo,$update_values,"id='".$QuickInvoiceInfoId."'");
				}
				
				if($QuickInvoiceInfoId!=0)
				{
						// is Old item available then add stock back
						$old_items=$db->rp_getData($ctableQuickPurchaseInvoiceItem,"*","invoice_id='".$QuickInvoiceInfoId."'");
						if($old_items)
						{
							while($old_item=mysql_fetch_assoc($old_items))
							{
								...
								$isUpdated=$db->rp_update($ctableQuickItemMaster,array("stock_qty"=>$new_stock),"aid='".$old_item['item_id']."'",0);
							}
						}
						$db->rp_delete($ctableQuickPurchaseInvoiceItem,"invoice_id='".$QuickInvoiceInfoId."'");
						foreach($Items as $Item)
						{
							...
							if($item_description!="")
							{
								...
								$ItemInsertedId=$db->rp_insert($ctableQuickPurchaseInvoiceItem,$value_invoice_item,$columns_invoice_item,0);
								if($ItemInsertedId!=0)
								{
									...
									echo $new_stock=$current_stock+$item_qty;
									// Update New Stock;
									$isUpdated=$db->rp_update($ctableQuickItemMaster,array("stock_qty"=>$new_stock),"aid='".$item_id."'",0);
						...
						$IsQuickInvoiceUpdated=$db->rp_update($ctableQuickPurchaseInvoiceInfo,$UpdateInvoice,"id='".$QuickInvoiceInfoId."'");
		...
		catch(Exception $e)
		{
			...
			// Revert All Entries
			$db->rp_delete($ctableQuickPurchaseInvoiceInfo,"id='".$QuickInvoiceInfoId."'");
			$db->rp_delete($ctableQuickPurchaseInvoiceItem,"invoice_id='".$QuickInvoiceInfoId."'");
		}	
	...
	if($reply['ack']==1)
	{
		...
		$db->rp_location("quick_purchase_invoice_view.php?m=e&iid=".$QuickInvoiceInfoId);
	}
	else
	{
...
if($_REQUEST['mode']=="edit")
{
		...
		if(isset($_REQUEST['iid']) && $_REQUEST['iid']!="")
		{
			...
			$invoice=$db->rp_getData($ctableQuickPurchaseInvoiceInfo,"*","id='".$invoice_id."'");
			if($invoice)
			{
				...
				$invoice_items_r=$db->rp_getData($ctableQuickPurchaseInvoiceItem,"*","invoice_id='".$invoice_id."'");
	...
	catch(Exception $e)
	{
		...
		$db->rp_location("quick_purchase_invoice_manage.php");
	}
}
                            ...
                            <div class="page-head">
                                <div class="container-fluid">
                                    ...
                                    <div class="page-title">
                                        <h1><a href="<?php echo "quick_purchase_invoice_manage.php" ?>" class="btn btn-default active"><i class="fa  fa-arrow-circle-o-left"></i>&nbsp;Back</a> &nbsp;<?php echo $page_title; ?></h1>
                                    </div>
                                    <!-- END PAGE TITLE -->                                   
                                </div>
                            </div>
                                ...
                                <div class="portlet light portlet-fit bordered">
                                    <div class="portlet-title">
                                        <div class="caption">
                                            ...
                                            <span class="caption-subject font-red sbold uppercase">Purchase Invoice</span>
                                        </div>
										<div class="actions">
										
											<div class="btn-group btn-group-devided">
													<a name="options" href="quick_purchase_invoice_manage.php" class="btn btn-transparent blue btn-outline btn-square btn-sm active" id="back-invoice-btn" type="button">Back</a>
										 ...
										 <div class="row">
											...
											<div class="form-group">
												...
												<div class="input-group">
													...
													<input data-validation="required" data-validation-error-msg="Vendor Name Required" class="form-control" placeholder="Vendor Name" id="customer_name" name="customer_name" type="text" value="<?php echo $customer_name; ?>"> 
										   ...
										   <div class="col-sm-6">
												<div class="form-group">
													<div class="col-sm-5"><label class="lable-inline"><b>State of vendor</b></label></div>
													<div class="col-sm-7">
...
$(function(){
 ...
 function assignAutoComplete(node) {
		 $(node).autocomplete({	
			  ...
			  select: function( event, ui ) {
				...
				$(unit_price_input).val(ui.item.unit_cost);
				...
				if(place_of_supply=="GUJARAT")
				{
					...
					$(cgst_input).attr("data-val",(ui.item.igst_rate));
					$(sgst_input).attr("data-val",(ui.item.igst_rate));
				}
				else
				{
					var cgst_input=$(parent).find('select[name="line_items['+index+'][cgst_rate]"]');
 ...
 function createNewRow(row_count){
	 var tableRow='<tr class="error-container'+row_count+'" data-count="'+row_count+'">'+
					...
					'<td><input autocomplete="off" id="line_items['+row_count+'][quantity]" name="line_items['+row_count+'][quantity]" value="" placeholder="" class="form-control table-input text-right numeric-input item-qty" type="text"></td>'+
