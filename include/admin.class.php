		...
		if($user_id!=0)
		{
			...
			if($result_admins)
			{
				while($detail=mysql_fetch_assoc($result_admins))
				{
					...
					$detail['countMessage']=$this->db->rp_getTotalRecord("chat_message","receiver='".$user_id."' AND sent_by=0");
					$detail['wallet_amount']=$this->db->rp_getValue("user_wallet","amount","uid='".$user_id."'");
					$params=array("participant_first"=>$user_id,"participant_second"=>$detail['id'],"participant_first_type"=>0,"participant_second_type"=>0);
					$chat_info=$this->chat->getChatRoom($params,true,true,false);					
					$detail['chat']=$chat_info;
