...
class feed extends Functions
{
	...
	public $unique_column="feed_title";
	// Public Varibale
	public $id='';public $feed_title='';public $feed_url='';public $feed_media_path='';public $feed_media_type='';public $feed_datetime='';public $feed_origin='';
	public $valid_keys=array("id","feed_title","feed_url","feed_media_path","feed_media_type","feed_datetime","feed_origin");
	...
	function view($where="",$required_columns=array(),$orderby="",$limit="")
	{
		...
		if($this->rights['view_flag']==1)
		{
			...
			{
				$reply=array("ack"=>0,"developer_msg"=>"Feed detail can’t be fetch!","ack_msg"=>"Feed detail can’t be fetch!");
				return $reply;
			}
		}
		else
	...
	function insert($detail=array(),$dup_check_array=array())
	{
		...
		{
			// id,feed_title,feed_url,feed_media_path,feed_media_type,feed_datetime,feed_origin		
			...
			if($validateKey['ack']==1)
			{
				...
				if(!empty($detail))
				{
					...
					$count=$this->countfeed($dup_check_array['key'],$dup_check_array['value']);
					else
					$count=0;	
					if($count<=0)
					{
						...
						if($inserted_id!=0)
						{
							$reply=array("ack"=>1,"developer_msg"=>"Feed detail inserted successfully!!","ack_msg"=>"Feed detail inserted successfully!!");
							return $reply;
						}
						else
						{
		...
		if($this->rights['update_flag']==1)
		{
			...
			if($validateKey['ack']==1)
			{
				...
				if(!empty($detail)&& $primary_key!="")
				{
					...
					$count=$this->countfeed($this->primary_column,$primary_key);
					if($count>=1)
					{	
						$count=$this->duplicatefeed($this->unique_column,$unique_key,$primary_key);
						if($count<=0)
						{
							...
							if($isUpdated)
							{
								$reply=array("ack"=>1,"developer_msg"=>"Feed detail updated successfully!!","ack_msg"=>"Feed detail updated successfully!!");
								return $reply;
							}
							else
							{
								$reply=array("ack"=>0,"developer_msg"=>"Feed detail can’t be updated!!","ack_msg"=>"Feed detail can’t be updated!!");
								return $reply;
							}
						}
						else
	...
	function delete($delete_array)
	{
		if($this->rights['delete_flag']==1)
		{
				...
				$count=$this->countfeed($delete_array['key'],$delete_array['value']);
				if($count>=1)
				{
		...
		{
			$this->db->rp_location("access_denied.php?msg=delete");
			$reply=array("ack"=>0,"developer_msg"=>"Feed detail can’t be updated!!","ack_msg"=>"Feed detail can’t be updated!!");
			return $reply;
		}		
	}
	...
	function active($active_array)
	{
		if($this->rights['update_flag']==1)
		{
			$count=$this->countfeed($active_array['key'],$active_array['value']);
			if($count>=1)
			{
				$this->db->rp_update($this->ctable,array("isActive"=>$active_array['status']),$active_array['key']."=".$active_array['value']);
				$reply=array("ack"=>1,"developer_msg"=>"Feed detail updated successfully!!","ack_msg"=>"Feed detail updated successfully!!");
				return $reply;
			}
			else
		...
		{
			$this->db->rp_location("access_denied.php?msg=update");
			$reply=array("ack"=>0,"developer_msg"=>"Feed detail can’t be updated!!","ack_msg"=>"Feed detail can’t be updated!!");
			return $reply;
		}		
	}
	...
	function countfeed($key,$value)
	{		
		$countfeed = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
		return $countfeed;	
	}
	function duplicatefeed($key,$value,$primary_key)
	{		
		$countfeed = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND ".$this->primary_column."!=".$primary_key,0);
		return $countfeed;	
	}
